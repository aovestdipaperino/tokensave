...
Manual test queries for verifying tokensave MCP tools. Run these in a Claude Code session after `tokensave init` and `tokensave install`.
...
tokensave_config(path="a", glob="b", key="x")        # both path and glob → error
tokensave_config(path="a")                           # missing key → error
```

---

## tokensave_signature_search (5.0)
...
Pattern-matching sites (`match Foo { ... }`, `if let Foo { ... }`) are filtered out, as are definition sites (`struct Foo { ... }`, `impl Foo { ... }`, `-> Foo {`).
...
```
Expected: Returns `{field, qualifier, write_count, read_count, write_sites: [...], read_sites: [...]}`. Writes include simple assignments (`x.field = ...`), compound assignments (`x.field += ...`), and `&mut x.field` borrows. Everything else is a read; `==` and `=>` do NOT count as writes.
...
```
Expected: The `qualifier` field carries `"GraphStats"`. The actual scan still uses the bare field name (we can't disambiguate `.foo` to a specific struct without type info), so qualified form is informational.
...
## LSP-resolved edges (5.0)

When LSP servers are installed, cross-file edges are resolved with semantic accuracy by `rust-analyzer`, `gopls`, `clangd`, `zls`, `lua-language-server`, `pyright-langserver`, or `jdtls`. Edges resolved this way carry `resolved_by='lsp'` in the v8 schema; heuristic-resolved edges carry `'heuristic'`; extractor-emitted edges carry `'direct'`.

Test the kill switch:
```
TOKENSAVE_LSP=0 tokensave sync          # skip the LSP pass entirely
tokensave sync --no-lsp                 # equivalent CLI flag
```
Expected: The sync output reports `0 via LSP` regardless of which servers are installed.

Test diagnostic visibility:
```
sqlite3 .tokensave/tokensave.db "SELECT resolved_by, COUNT(*) FROM edges GROUP BY resolved_by"
```
Expected: Counts grouped into `direct`, `heuristic`, and (if LSP servers were active) `lsp`.
