...
### Added
- **Index Godot `.gdshader` shader files as GLSL (#270).** `.gdshader` now maps to the existing GLSL extractor, so shader uniforms, functions, and structs are indexed and searchable like any other source file. Godot's `: hint...` uniform annotations (`uniform sampler2D albedo : source_color;`, `uniform float x : hint_range(0,1) = 0.5;`) are not valid GLSL and previously caused the grammar to mis-attribute the declared name; the hint clause is now blanked out (length-preserving, so node positions are unaffected) before parsing, so hinted uniforms resolve to correct `Const` nodes. Godot's `global uniform`/`instance uniform` qualifiers, which also have no GLSL equivalent, are normalized the same way (the leading `global `/`instance ` keyword is blanked) so those declarations resolve to `Const` too, instead of `Static`. Node signatures and excerpts are built from the original, un-blanked source, so a hinted or `global`/`instance` uniform's signature still shows the real source text. `#include` directives in `.gdshader` files are indexed via the existing GLSL include handling.
