...
- **`tokensave upgrade` no longer offers to kill its own launcher, and no longer proceeds if a requested kill leaves a process running.** Candidate selection now excludes this process and every reachable parent in the process tree (e.g. a Scoop shim or shell that launched `tokensave`), so it can't be listed as a killable "other" process; unrelated `tokensave` MCP servers are still offered. If `--kill`/an interactive `y` doesn't stop every listed process, the upgrade now aborts instead of continuing, telling the user to stop them manually and retry.
