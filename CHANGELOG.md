...
### Added
- **LSP-resolved cross-file edges** — new `src/lsp/` module spawns Phase 1 language servers (`rust-analyzer`, `gopls`, `clangd` for C / C++ / Obj-C, `zls`, `lua-language-server`, `pyright` / `pylsp`, `jdtls`) and runs `textDocument/definition` for every unresolved reference before the heuristic resolver kicks in. Adapter startup is concurrent via `tokio::task::JoinSet`, so a multi-language project pays the cost of the slowest server (rust-analyzer at ~10s) instead of the sum across every adapter. Per-adapter failures are non-fatal: missing binaries or manifests just fall through to heuristic resolution. Edges resolved by the LSP pass carry `resolved_by='lsp'`; heuristic edges carry `'heuristic'`; extractor-emitted edges carry `'direct'`.
- **`TOKENSAVE_LSP=0` env var and `tokensave sync --no-lsp` flag** — kill switches for the LSP pass. Set either to skip the resolver layer entirely; behavior reverts to 4.x heuristic-only.
- **v8 schema migration** — adds `edges.resolved_by` (NOT NULL DEFAULT `'direct'`) plus an index, and a new `read_cache` table backing `tokensave_read`. Forward-only; runs inside an EXCLUSIVE transaction. Existing graphs migrate transparently on the next sync.
- **`tokensave_read` tool** — mode-aware file read (`full`, `lines`, `map`, `signatures`) with cross-session response cache. Repeat calls on an unchanged file return a 30-token `{"unchanged": true, ...}` stub. `map` and `signatures` modes serve from the graph (no source bytes touched). For graph-backed modes the cache key includes `last_sync_at`, so a force-reindex correctly invalidates stale rows even when file mtime is unchanged.
- **`tokensave_outline` tool** — flat list of every top-level symbol in a file (kind, name, line range, visibility) with optional `kinds` filter. Cheapest replacement for the `grep -n "^pub fn\|^pub struct\|..."` pattern (618 hits in the 2026-05-04 telemetry scan).
- **`tokensave_implementations` tool** — every type implementing a given trait (with each impl's method bodies), or every function/method of a given name across the project, with bodies.
- **`tokensave_unsafe_patterns` tool** — finder for `unwrap`, `expect`, `panic!`, `todo!`, `unimplemented!`, and `unsafe { }` blocks. Each match has file, line, kind, enclosing symbol, snippet, and an `in_test` flag derived from path heuristics. Word-boundary matching prevents `.unwrap_or` false positives. Comment-line filter skips `//`-prefixed lines.
- **`tokensave_diagnostics` tool** — runs the project's compile / type checker (`cargo check --message-format=json` for Rust, `tsc --noEmit --pretty false` for TypeScript, `pyright --outputjson` for Python), parses structured output, and maps each diagnostic to the smallest enclosing graph node. Cargo runs against `.tokensave/target/` to avoid racing with interactive cargo runs. TypeScript and Python drivers fall through silently when their binaries aren't installed; the Rust driver fails hard since cargo is universal in Rust dev environments. Largest single Bash : tokensave gap from the telemetry (777 invocations).
- **`tokensave_config` tool** — TOML and JSON config-file queries by dotted-path (`package.version`, `compilerOptions.target`, `dependencies.tokio`). Single-file via `path` or multi-file via `glob`. The only tool in the surface that doesn't query the code graph; works on uninitialized projects.
- **`tokensave_signature_search` tool** — find functions and methods by signature shape: return type substring, parameter substrings, async-ness, path scope. Filters compose with AND. Searches the cached `signature` column on every Function/Method node.
- **`tokensave_constructors` tool** — every literal-instantiation site (`Foo { ... }`) of a named struct, with the field list present at each site and the set of fields missing relative to the struct's current definition. Handles Rust struct-update syntax (`..base`), match-arm patterns, if-let / while-let patterns, lifetime annotations, char literals, and string-literal context. After adding a required field, surfaces every site that needs updating before cargo even compiles.
- **`tokensave_field_sites` tool** — partition every `.field` reference across the codebase into reads and writes. Recognises simple assignment, compound assignment (`+=`, `-=`, ...), and `&mut` borrows as writes. `==` and `=>` correctly count as reads. The write list is the exact blast radius for field renames and invariant additions.
- **`lsp-integration` Cargo feature** — gates the new `tests/lsp_integration_test.rs` integration tests that spawn a real `rust-analyzer` against a temporary fixture crate. Off by default so `cargo test` works on hosts without LSP servers installed; enable with `cargo test --features lsp-integration`.
- **`Database::insert_edges_with_provenance(edges, source)`** — the new edge-insert API used by the LSP, heuristic, and direct-extraction paths to tag each edge's origin into the v8 `resolved_by` column. `insert_edges` is now a thin wrapper that delegates with `source = "direct"`.
- **Multi-manifest support on `LspAdapter`** — new `manifest_present(project_root)` trait method with a default that mirrors the prior single-filename check. `JavaAdapter` overrides to accept any of `pom.xml`, `build.gradle`, `build.gradle.kts`, or `.classpath`, so jdtls now spawns on Gradle and bare-classpath Java projects too.
- **`docs/5.0-RELEASE-PLAN.md`, `docs/5.0-DESIGN.md`, `docs/LSP-INTEGRATION.md`, `docs/LEAN-CTX-COMPARE.md`, `docs/MCP-extensions.md`** — release plan, detailed design, LSP architecture, design provenance for `tokensave_read`, and the telemetry-backed selection of the new MCP tools.
- **`README.md` Language Support Matrix** — per-feature support table for the 11 lite-tier languages, covering tree-sitter extraction, heuristic edges, LSP-resolved edges, `tokensave_diagnostics` driver, edit primitives, and pattern tools.
- **`TEST-QUERIES.md` entries for every new tool** — manual-test queries plus an LSP section covering kill-switch usage and how to inspect `resolved_by` counts directly via sqlite.

### Changed
- **Tool count: 52 → 61.** Three core tools (`tokensave_search`, `tokensave_context`, `tokensave_status`) remain `anthropic/alwaysLoad`; everything else is read-only and safe to call in parallel.
- **`tokensave wipe` no longer leaks the global DB into the wipe set when `$HOME` is symlinked** — the home `.tokensave` skip relied on lexical path equality, so a user whose `$HOME` resolves through a symlink (e.g. macOS `/Users/x` vs the canonical `/private/var/...`) could see `~/.tokensave` show up as a wipe target if the descendant walk reached it via the canonical chain. The skip now canonicalizes both the home path and every candidate before comparing. (Forward-merged from master.)
- **Descendant walk for `tokensave wipe` / `tokensave list` is now iterative with cycle protection** — `find_descendant_tokensave` used to recurse; it now uses an explicit worklist plus a canonical-path `visited` set, so the walk is bounded even if a directory cycle slips past the symlink filter (e.g. Windows junctions). (From master.)
- **`tokensave doctor` purges stale global-DB entries in batched statements** — a new `GlobalDb::delete_projects(&[String])` issues one `DELETE ... WHERE path IN (...)` per chunk of 256, so a 216-row purge is one round-trip instead of 216 serial deletes. (From master.)
- **`gather_local_projects_from` is now a separately-exported helper** — extracts the pure discovery logic from the cwd-driven wrapper so the ancestor + descendant walk can be unit-tested without mutating the process's working directory. (From master.)
- **Cleared `clippy::map_unwrap_or` warning in `display::shuffle_flags`** — the xorshift seed now uses `map_or` instead of `map(...).unwrap_or(...)`. Behavior unchanged. (From master.)

### Fixed
- **LSP client no longer leaks pending-map entries on serialise / send / oneshot failures** — `LspClient::request` reorders work so param serialisation runs before the pending insert, and every error path removes the pending entry inline before returning. The previous `tokio::spawn` cleanup on timeout could race with the reader; serial cleanup eliminates that window.
- **`find_struct_literals` pattern-stack pop now uses `==` instead of `<=`** — the looser inequality popped outer pattern contexts when an inner pattern's brace closed (e.g. `match x { Some(_) => match y { ... } }`). Strict equality fixes the latent off-by-one.
- **`find_struct_literals` skips string and char literals** — a struct shape inside `"Foo { x: 1 }"` no longer fires as a constructor site. Char-literal detection distinguishes `'a'` (closing apostrophe within identifier range) from lifetime annotations like `'static` (no closing apostrophe).
- **`handle_diagnostics` hoists `get_nodes_by_file` out of the per-diagnostic loop** — a multi-error file (a missing import cascading) used to do N round-trips per file; one round-trip now suffices via a `HashMap<file, Vec<Node>>`.
