...
### Fixed
- **`tokensave_simplify_scan` no longer treats same-named Rust methods as duplicated implementations (#244).** Fuzzy search relevance was being used as duplication evidence, so conventional methods such as `validate`, `seal`, and `computed_digest` on unrelated receiver types were reported together, sometimes alongside similarly named helpers. Simplify scan now uses exact leaf names only to bound candidates and reports only non-empty, exact source-body hash matches, with module/receiver-qualified identities and deterministic ordering. Renamed, reformatted, and structural near-duplicates remain available through `tokensave_redundancy`.
