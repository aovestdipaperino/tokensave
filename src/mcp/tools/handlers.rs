...
///
/// `{ "trait": "Foo" }` walks the graph from every Trait/Interface node named
/// `Foo` along incoming `Implements` edges, then for each implementor fetches
/// the methods it `Contains` and returns their bodies. `{ "method": "bar" }`
/// is the simpler case: every Function/Method node named `bar`, with body.
async fn handle_implementations(
    ...
    if let Some(name) = trait_name {
        // Find every trait/interface/class node with this name. Tree-sitter
        // language extractors can disagree on whether a Java/TS interface
        // is `Trait` or `Interface`, so accept all type-defining kinds and
        // filter further by name match.
        let candidates = cg
            .db()
...
/// Collect the method bodies that an implementing type contains.
async fn collect_method_bodies(
    ...
    for edge in outgoing {
        ...
        let body = match crate::sync::read_source_file(&abs_path) {
            Ok(source) => extract_lines(&source, child.start_line, child.end_line),
            Err(_) => String::from("<file unreadable>"),
        };
        out.push(json!({
            "name": child.name,
