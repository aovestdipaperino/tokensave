...
async fn record_and_query_savings_round_trip() {
    ...
    db.record_savings("/proj/a", "tokensave_context", 10_000, 500, now).await;
    db.record_savings("/proj/a", "tokensave_search", 2_000, 100, now + 60).await;
    db.record_savings("/proj/b", "tokensave_context", 5_000, 250, now + 120).await;
...
async fn savings_history_buckets_by_day() {
    ...
    db.record_savings("/proj/a", "tokensave_context", 1000, 100, day1).await;
    db.record_savings("/proj/a", "tokensave_context", 500, 50, day1 + 3600).await;
    db.record_savings("/proj/a", "tokensave_context", 800, 80, day2).await;
