...
fn test_glsl_extract_functions() {
    ...
    assert!(fn_names.contains(&"fresnelSchlick"), "functions: {fn_names:?}");
    assert!(fn_names.contains(&"distributionGGX"), "functions: {fn_names:?}");
    assert!(fn_names.contains(&"geometrySchlickGGX"), "functions: {fn_names:?}");
    assert!(fn_names.contains(&"calculatePointLight"), "functions: {fn_names:?}");
}
...
fn test_glsl_extract_structs() {
    ...
    assert!(struct_names.contains(&"PointLight"), "structs: {struct_names:?}");
    assert!(struct_names.contains(&"Material"), "structs: {struct_names:?}");
}
...
fn test_glsl_extract_struct_fields() {
    ...
    assert!(field_names.contains(&"intensity"), "fields: {field_names:?}");
    ...
    assert!(field_names.contains(&"roughness"), "fields: {field_names:?}");
}
...
fn test_glsl_extract_uniforms() {
    ...
    assert!(const_names.contains(&"uModelMatrix"), "consts: {const_names:?}");
    assert!(const_names.contains(&"uViewMatrix"), "consts: {const_names:?}");
    assert!(const_names.contains(&"uProjectionMatrix"), "consts: {const_names:?}");
...
fn test_glsl_extract_in_out_declarations() {
    ...
    assert!(field_names.contains(&"aPosition"), "fields: {field_names:?}");
    assert!(field_names.contains(&"aNormal"), "fields: {field_names:?}");
    assert!(field_names.contains(&"aTexCoord"), "fields: {field_names:?}");
    assert!(field_names.contains(&"vWorldPos"), "fields: {field_names:?}");
    assert!(field_names.contains(&"vNormal"), "fields: {field_names:?}");
    assert!(field_names.contains(&"vTexCoord"), "fields: {field_names:?}");
}
...
fn test_glsl_extract_preproc_defines() {
    ...
    let consts: Vec<_> = result.nodes.iter().filter(|n| n.kind == NodeKind::Const).collect();
    let const_names: Vec<_> = consts.iter().map(|n| n.name.as_str()).collect();
    assert!(const_names.contains(&"MAX_LIGHTS"), "consts: {const_names:?}");
}
...
fn test_glsl_function_docstrings() {
    ...
    let result = GlslExtractor.extract("sample.glsl", &source);
    assert!(result.errors.is_empty(), "errors: {:?}", result.errors);
    let fresnel = result.nodes.iter().find(|n| n.name == "fresnelSchlick").unwrap();
    assert!(fresnel.docstring.is_some(), "fresnelSchlick should have a docstring");
    assert!(
        fresnel.docstring.as_ref().unwrap().contains("Fresnel-Schlick"),
        "docstring: {:?}",
        fresnel.docstring
    );
}
...
fn test_glsl_function_signatures() {
    ...
    let result = GlslExtractor.extract("sample.glsl", &source);
    assert!(result.errors.is_empty(), "errors: {:?}", result.errors);
    let dist = result.nodes.iter().find(|n| n.name == "distributionGGX").unwrap();
    let sig = dist.signature.as_ref().unwrap();
    assert!(sig.contains("float"), "signature should include return type: {sig}");
    assert!(sig.contains("vec3 N"), "signature should include params: {sig}");
}
...
fn test_glsl_call_sites() {
    ...
    assert!(call_names.contains(&"fresnelSchlick"), "calls: {call_names:?}");
    assert!(call_names.contains(&"distributionGGX"), "calls: {call_names:?}");
    assert!(call_names.contains(&"geometrySchlickGGX"), "calls: {call_names:?}");
    // main calls calculatePointLight
    assert!(call_names.contains(&"calculatePointLight"), "calls: {call_names:?}");
}
...
fn test_glsl_complexity_metrics() {
    ...
    let calc = result.nodes.iter().find(|n| n.name == "calculatePointLight").unwrap();
    assert!(calc.branches > 0, "calculatePointLight should have branch complexity");
